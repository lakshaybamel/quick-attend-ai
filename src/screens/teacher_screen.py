...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning(
            "No subjects found! Please create a subject first in Manage Subjects tab."
        )
        return
    ...
    if st.session_state.attendance_images:
        st.header("Attendance Photos Gallery")
    ...
    with c2:
        ...
        if st.button(
            ...
            with st.spinner("Scanning photos for attendance..."):
                all_detected_ids = {}
...
def teacher_tab_manage_subjects():
    ...
    with col2:
        if st.button("Create Subject", width="stretch", icon=":material/add_circle:"):
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn(name=sub["name"], code=sub["subject_code"]):
                if st.button(
                    f"Share Code: {name}", key=f"share_{code}", icon=":material/share:"
                ):
        ...
        st.info(
            "No subjects found! Please create a subject to start taking attendance and enrolling students."
        )
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter username", placeholder="your username here")

    teacher_pass = st.text_input(
        "Enter password", type="password", placeholder="your password here"
    )
...
def teacher_screen_register():
    ...
    st.header("Register New Teacher Account", text_alignment="center")

    st.space()
    st.space()

    teacher_username = st.text_input(
        "Enter username", placeholder="your username here (used for login)"
    )

    teacher_name = st.text_input("Enter full name", placeholder="your full name here")
